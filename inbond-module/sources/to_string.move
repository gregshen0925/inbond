...
/// @title to_string
/// @notice `u64` to `String` conversion utilities.
module injoy_labs::to_string {
    use std::string::{Self, String};
    use std::vector;

    const HEX_SYMBOLS: vector<u8> = b"0123456789abcdef";

    // Maximum value of u64, i.e. 2 ** 128 - 1
    ...
    /// Converts a `u64` to its `string::String` hexadecimal representation.
    public fun to_hex_string(value: u64): String {
        if (value == 0) {
    ...
    public fun to_hex_string_fixed_length(value: u64, length: u64): String {
        let buffer = vector::empty<u8>();
